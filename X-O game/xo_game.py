def DisplayBoard(board): # to display the board for every move
	print("+-------" * 3,"+",sep="")
	for row in range(3):
		print("|          "*3,"|",sep="")
		for col in range(3):
...
def EnterMove(board): # storing the move (and checking whether its                                                  valid or not)
	ok = False	# initial condition to run the loop
	while not ok:
		move = input("Enter your move: ") 
		ok = len(move) == 1 and move >= '1' and move <= '9' # whether the user's input valid?
		if not ok:
			print("Bad move - repeat your input!") # no, it isn't - do the input again
			continue
		move = int(move) - 1 	# box's number from 0 to 8
		row = move // 3 	#  row
		col = move % 3		# column
